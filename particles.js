// Recursive particles sketch with deterministic seed and rich controls.
...
const particleSketch = (p) => {
  ...
  function applyUI() {
    ...
    params.zoom = (parseInt(zoomEl?.value || '150', 10)) / 100;
  ...
  function bindControls() {
    const ids = [
      ...
      'pr-depth', 'pr-branches', 'pr-radius', 'pr-id', 'pr-cell', 'pr-zoom',
      'pr-motion', 'pr-rot-freq', 'pr-rad-freq', 'pr-b1', 'pr-b2', 'pr-b3',
    ...
    if (randomPaletteBtn) {
      randomPaletteBtn.addEventListener('click', () => {
        params.pa = p.random(0.1, 0.6);
        params.pb = p.random(0.1, 0.6);
        params.pc = p.int(p.random(1, 5));
        params.pd = p.random();
        const paEl = byId('pr-pa');
        const pbEl = byId('pr-pb');
        const pcEl = byId('pr-pc');
        ...
        const heEl = byId('pr-hue-end');
        if (hsEl) hsEl.value = String(Math.floor(p.random(360)));
        if (heEl) heEl.value = String(Math.floor(p.random(360)));
        applyUI();
        p.redraw();
      });
    }
  ...
  p.draw = () => {
    ...
    const cellW = (minWidth * params.cellScale * params.zoom) / 2;
  ...
  function palette(a, b, c, d, t) {
    return a + b * p.cos(p.TWO_PI * (c * t + d));
  }
  ...
  function drawNode(width, id, depth) {
    ...
    const sw = Math.max(0.5, params.strokeWidth);
